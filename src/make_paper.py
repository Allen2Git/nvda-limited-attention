'''Generate the main NVDA limited-attention paper (~3000 Chinese chars,
prose only, no code snippets).  Uses triple-single-quoted strings so we
can safely include straight ASCII double quotes as string content if needed.
'''
import json
import os
import pandas as pd
...
r = t.add_run('有限注意力假说在 NVIDIA 上失效了吗？\n'
              '——基于 24 次财报事件的分钟级事件研究')
r.bold = True
r.font.size = Pt(16)
...
sub.add_run('清华-Cornell MBA · 数据分析与决策 II · 案例研究').font.size = Pt(11)
para('')
...
para(
    ...
    '使用分钟级 tick 聚合数据构造事件日的三段收益——'
    '盘前隔夜收益、开盘 1 小时收益、盘中剩余收益，'
...
para('关键词：有限注意力、财报事件、盘前交易、市场效率、NVIDIA',
     italic=True)
...
h('3. 研究设计', 1)
para(
    '本文的核心计量模型为一元加控制变量的截面回归：'
)
para('r_mid_day_i = α + β₁·r_open_1h_i + β₂·r_overnight_i + ε_i',
...
doc.add_picture(os.path.join(FIG, 'fig1_means_by_window.png'), width=Inches(6.2))
para('图 1  三个窗口的平均收益对比（财报日 vs 非财报日）',
     italic=True, align=WD_ALIGN_PARAGRAPH.CENTER)
doc.add_picture(os.path.join(FIG, 'fig4_overnight_fullday.png'), width=Inches(5.8))
para('图 2  隔夜收益对全日收益的线性拟合（斜率接近 1，R² 极高）',
     italic=True, align=WD_ALIGN_PARAGRAPH.CENTER)
...
tab = doc.add_table(rows=1, cols=7); tab.style = 'Light List'
for i, hd in enumerate(['设置', '变量', '系数', 'SE (HC1)', 't', 'p', 'R²']):
    tab.rows[0].cells[i].text = hd
...
para('表 1  核心回归、单变量回归、窗口敏感性与非财报日 placebo 的系数对比',
     italic=True, align=WD_ALIGN_PARAGRAPH.CENTER)
para(
    '值得特别注意的是 placebo 行：在 70 个完全无事件的普通交易日上，'
    f'β₁ 的估计值为 {reg[(reg["label"]=="placebo")&(reg["variable"]=="r_open_1h")]["coef"].iloc[0]:+.3f}，'
    '与财报日的 +0.06 几乎一致。这表明我们测到的零结果不是'
    '"样本量过小导致效应被噪声遮蔽"，而是"事件效应本身就与随机日无异"。'
    '这是本文最硬的识别证据。'
)
...
doc.add_picture(os.path.join(FIG, 'fig5_event_decomposition.png'), width=Inches(6.4))
para('图 3  每次财报的收益分解（隔夜 + 开盘 1 小时 + 盘中）',
     italic=True, align=WD_ALIGN_PARAGRAPH.CENTER)

h('4.4 EPS Surprise 分组', 2)
para(
    '我们按 EPS surprise 将 24 个事件分成三个等容量组（每组 n = 8），'
    '检验"过冲 + 反转"是否仅在大超预期子样本上存在。'
    '三组的开盘 1 小时与盘中收益的相关系数分别为 +0.21、-0.71、+0.45。'
    '中等 surprise 组的 -0.71 看似显示了反转，但在 n = 8 的样本下统计力不足；'
    '高 surprise 组的相关系数反而为正 +0.21。'
    '综合来看，没有任何子样本呈现系统性"过冲 + 反转"的证据。'
)

# ============ 5 机制解释 ============
h('5. 为什么有限注意力假说在 NVDA 上失效？', 1)
para(
...
para(
    '对普通散户而言，最直接的启示是：'
    '长期持有策略不会被本文结果所影响——如果投资逻辑是基于 AI 需求的多年叙事，'
...
para(
    ...
    '跨代复刻研究（replication across eras）'
    '是未来重要的研究方向，有助于我们理解哪些行为金融规律'
    '依赖于特定的市场结构前提。'
)
...
refs = [
    ...
    'and financial reporting. Journal of Accounting and Economics, 36(1–3), 337–386.',
    'Kelley, E. K., & Tetlock, P. C. (2013). How wise are crowds? Insights from retail '
    'orders and stock returns. Journal of Finance, 68(3), 1229–1265.',
    'Lee, C. M. C. (1992). Earnings news and small traders: An intraday analysis. '
    'Journal of Accounting and Economics, 15(2–3), 265–302.',
